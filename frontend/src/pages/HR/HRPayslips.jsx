...
const HRPayslips = () => {
  ...
  const [filterMonth, setFilterMonth] = useState('');
  const [filterYear, setFilterYear] = useState('');
  const [filterEmployee, setFilterEmployee] = useState(''); // employeeId (_id)
  const [form, setForm] = useState({
    employeeId: '', 
  ...
  const filteredPayslips = filterEmployee
    ? payslips.filter(ps => ps.employee?._id === filterEmployee)
    : payslips;

  const sortedPayslips = filteredPayslips;
    ...
    <div className="max-w-6xl mx-auto px-3 sm:px-4 space-y-4 sm:space-y-5 animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{sortedPayslips.length} of {payslips.length} payslips</p>
        </div>
      ...
      <Card>
        <div className="flex flex-wrap gap-3 mb-4">
          <select className="input-field w-auto" value={filterEmployee} onChange={e => setFilterEmployee(e.target.value)}>
        ...
        {sortedPayslips.length === 0 ? (
          <EmptyState
            ...
            title="No payslips"
            message={filterEmployee ? 'No payslips found for this employee.' : 'Generate payslips for employees.'}
            action={!filterEmployee ? { label: 'Generate', onClick: () => setShowModal(true) } : undefined}
          />
        ) : (
          <>
            {/* Mobile cards */}
