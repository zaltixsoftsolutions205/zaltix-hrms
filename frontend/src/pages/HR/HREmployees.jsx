...
export default function HREmployees() {
  ...
  const handleCreate = async (ev) => {
    ...
    if (form.role === 'custom' && !form.customRole.trim()) {
      toast.error('Please enter a custom role name'); return;
    }
    setLoading(true);
    const payload = { ...form, role: form.role === 'custom' ? form.customRole.trim() : form.role };
    try {
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Add New Employee" size="lg">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <SelectField label="Role *" value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value, customRole: '' }))}>
                <option value="employee">Employee</option>
...
function EditForm({ emp, departments, onDone }) {
  ...
  const emailChanged = form.email.trim().toLowerCase() !== (emp.email || '').toLowerCase();
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const payload = { ...form, role: form.role === 'custom' ? form.customRole.trim() : form.role, sendNewCredentials };
    try {
      await api.put(`/employees/${emp._id}`, payload);
      toast.success(sendNewCredentials ? 'Employee updated! New credentials emailed.' : 'Employee updated!');
      onDone();
    }
