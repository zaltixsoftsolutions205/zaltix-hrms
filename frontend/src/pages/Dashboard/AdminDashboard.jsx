...
const AdminDashboard = () => {
  ...
  const [leadView, setLeadView] = useState('today'); // 'today' | 'week'
    ...
    <div className="max-w-[1400px] mx-auto px-3 sm:px-4 xl:px-6 py-2 space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.07 }}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
          <div>
            <p className="font-bold text-gray-900 text-sm">Leads</p>
            <p className="text-[11px] text-gray-400 mt-0.5">
              {(leadView === 'today'
                ? allLeads.filter(l => new Date(l.createdAt) >= dayStart)
                : allLeads.filter(l => new Date(l.createdAt) >= weekStart)
              ).length} leads
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Link to="/admin/crm" className="text-xs font-semibold text-violet-500 hover:text-violet-700">All →</Link>
          </div>
        </div>
        {(() => {
          const list = (leadView === 'today'
            ? allLeads.filter(l => new Date(l.createdAt) >= dayStart)
            : allLeads.filter(l => new Date(l.createdAt) >= weekStart)
          ).slice(0, 10);
          const statusColor = s => ({
          ...
          return (
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {list.map((l, i) => (
                    <tr key={l._id} className={`border-b border-gray-50 hover:bg-violet-50/30 transition-colors ${i % 2 === 1 ? 'bg-gray-50/30' : ''}`}>
                      <td className="px-5 py-3 font-semibold text-gray-800">{l.name || '—'}</td>
                      <td className="px-4 py-3 text-gray-500">{l.company || '—'}</td>
                      <td className="px-4 py-3 text-gray-500">{l.serviceType || '—'}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold capitalize ${statusColor(l.status)}`}>
                          {l.status}
                        </span>
                      </td>
