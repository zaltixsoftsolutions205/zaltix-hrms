...
const CRMPage = () => {
  ...
  // ============ LEADS CSV DOWNLOAD ============
  const downloadLeadsCSV = async (all = false) => {
    try {
      let leads = leadsData.leads;
      if (all && leadStatusFilter) {
        // fetch all leads without filter
        const res = await api.get('/leads');
        leads = res.data.leads || res.data;
      }
      const headers = ['Name', 'Phone', 'Email', 'Source', 'Status', 'Notes', 'Added Date', 'Last Updated'];
      ...
      a.href = url;
      const label = all ? 'all' : (leadStatusFilter || 'all');
      a.download = `leads_${label}_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success(`Downloaded ${leads.length} lead${leads.length !== 1 ? 's' : ''}`);
    } catch {
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
              ...
              <div className="filter-bar">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => downloadLeadsCSV(false)}
                    title={leadStatusFilter ? `Download filtered (${leadStatusFilter}) leads` : 'Download all leads'}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-violet-300 text-violet-700 rounded-lg hover:bg-violet-50 transition-colors">
                    <SI d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" size={13} />
                    {leadStatusFilter ? `Export (${leadStatusFilter.replace(/-/g, ' ')})` : 'Export CSV'}
                  </button>
                  {leadStatusFilter && (
                    <button onClick={() => downloadLeadsCSV(true)}
                      title="Download all leads regardless of filter"
