...
export const useAttendance = (onRefresh) => {
  ...
  const handleCheckIn = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get('/holidays', { params: { year: new Date().getFullYear() } });
      const holidays = Array.isArray(data) ? data : (data.holidays || []);
      const todayHoliday = holidays.find(h => h.date && h.date.slice(0, 10) === today);
      if (todayHoliday) {
        toast.error(`Today is a holiday: ${todayHoliday.name}. Check-in is not allowed.`, { duration: 5000 });
        return;
      }
    } catch (_) { /* if API fails, let backend block it */ }
    _openModal('checkin');
  }, [_openModal]);
