...
// Role aliases mapping from frontend display values to backend constants
const ROLE_ALIASES = {
  'Employee': ROLES.EMPLOYEE,
  'employee': ROLES.EMPLOYEE,
  'Sales Employee': ROLES.SALES,
  'Field Sales Executive': ROLES.SALES,
  'Sales Executive': ROLES.SALES,
  'sales executive': ROLES.SALES,
  'sales': ROLES.SALES,
  'HR': ROLES.HR,
  'hr': ROLES.HR,
...
const normalizeRole = (role) => {
  ...
  // Check if role exists in aliases (case-insensitive)
  const normalizedRole = ROLE_ALIASES[role] || 
                        ROLE_ALIASES[role.toLowerCase()] || 
                        ROLE_ALIASES[role.charAt(0).toUpperCase() + role.slice(1).toLowerCase()];
  
  return normalizedRole || ROLES.EMPLOYEE;
};
...
exports.getAllEmployees = async (req, res) => {
  try {
    ...
    if (req.user.role === ROLES.ADMIN) {
      // Admin can see all active employees except maybe other admins? 
      // Adjust based on your requirements
      filter.role = { $ne: ROLES.ADMIN }; // Hide other admins from list
    } else if (req.user.role === ROLES.HR) {
      // HR can see all non-admin employees (employees, sales, etc.)
      filter.role = { $ne: ROLES.ADMIN };
    } else {
      // Regular employees shouldn't access this function usually,
      // but if they do, show only themselves or empty array
    ...
    allowed.forEach(field => {
      if (req.body[field] !== undefined) {
        if (field === 'role') {
          ...
          employee[field] = normalizeRole(req.body[field]);
...
exports.deleteEmployee = async (req, res) => {
  try {
    ...
    if (employee.role === ROLES.ADMIN) return res.status(403).json({ message: 'Cannot delete an admin account' });
...
// Optional: Get role options for frontend dropdown
